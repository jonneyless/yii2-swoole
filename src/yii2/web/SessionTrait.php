 ...
 * 协程模式下的session处理类
 *
 * @see \web\cm\SessionTrait 非协程下的引用的类
...
trait SessionTrait
{
    ...
    {
        if ($this->getIsActive()) {
            YiiBuilder::debug('Session closed in swoole', __METHOD__);
    ...
    protected function newSessionId()
    {
        ...
        if (version_compare(PHP_VERSION, '7.1', '<')) {
            $sid = md5(SessionTrait . phpYii::$app->getRequest()->getUserIP() . microtime() . rand(0, 100000));
        } else {
...
//        @session_start();
        if ($this->getIsActive()) {
            YiiBuilder::info('Session started in swoole', __METHOD__);
