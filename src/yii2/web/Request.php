...
class Request extends \yii\web\Request
{
    ...
    {
        $result = YiiBuilder::$app->getUrlManager()->parseRequest($this);
        if ($result !== false) {
            [$route, $params] = $result;
        ...
        throw new NotFoundHttpException(YiiBuilder::t('yii', 'Page not found.'));
    }
    ...
    public function getBodyParams()
    {
        if ($this->_bodyParams === null) {
            ...
            if (isset($this->parsers[$contentType])) {
                $parser = YiiBuilder::createObject($this->parsers[$contentType]);
                if (!($parser instanceof RequestParserInterface)) {
                    throw new InvalidConfigException("The '$contentType' request parser is invalid. It must implement the yii\\web\\RequestParserInterface.");
                }
                $this->_bodyParams = $parser->parse($this->getRawBody(), $contentType);
            } elseif (isset($this->parsers['*'])) {
                $parser = YiiBuilder::createObject($this->parsers['*']);
                if (!($parser instanceof RequestParserInterface)) {
    ...
    protected function loadCookies()
    {
        ...
        if ($this->enableCookieValidation) {
            ...
            foreach ($this->getSwooleRequest()->cookie as $name => $value) {
                ...
                $data = YiiBuilder::$app->getSecurity()->validateData($value, $this->cookieValidationKey);
                if ($data === false) {
                    continue;
                }
