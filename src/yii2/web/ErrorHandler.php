...
class ErrorHandler extends \yii\web\ErrorHandler
{
    ...
    public function handleFallbackExceptionMessage($exception, $previousException)
    {
        ...
        if (YII_DEBUG) {
            $msg = '<pre>' . htmlspecialchars($msg, ENT_QUOTES, YiiBuilder::$app->charset) . '</pre>';
        } else {
            $msg = 'An internal server error occurred.';
        }
        $this->clearOutput();
        /** @var \swoole_http_response $res */
        $res = YiiBuilder::$app->response->getSwooleResponse();
        $res->end($msg);
    }
    ...
    public function handleError($code, $message, $file, $line)
    {
        if (error_reporting() & $code) {
            ...
            if (!class_exists('yii\\base\\ErrorException', false)) {
                require_once(YiiBuilder::getAlias('@yii/base/ErrorException.php'));
            }
