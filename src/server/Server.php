...
class Server
{
    ...
    /**
     * @var \bootstrap\BootstrapInterface
     */
    public $bootstrap;
    ...
    public function init()
    {
        switch ($this->serverType) {
            case 'http':
                $this->swoole = new Swoole\Http\Server($this->host, $this->port, $this->swooleMode, $this->sockType);
            ...
            default:
                ...
                $events = [
                    'ManagerStart', 'ManagerStop', 'PipeMessage', 'Packet',
...
//                posix_kill($masterPid, SIGUSR2); // reload all task
    ...
    static function autoCreate($nodeConfig)
    {
        if (isset($nodeConfig['class'])) {
            ...
            throw new Exception('class must implement jonneyless\swoole\server');
        } else {
